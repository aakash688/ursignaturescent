...
var handleError = /* @__PURE__ */ __name(async (error3, reject, options, namespace) => {
  if (error3 && typeof error3 === "object" && "status" in error3 && "ok" in error3 && typeof error3.status === "number") {
...
function getAdminClient(env2) {
  const url = env2.SUPABASE_URL || env2.NEXT_PUBLIC_SUPABASE_URL;
  const key = env2.SUPABASE_SERVICE_ROLE_KEY || env2.NEXT_PUBLIC_SUPABASE_ANON_KEY;
...
// src/lib/auth.ts
async function getAuthFromRequest(request, env2) {
  const cookie = request.headers.get("Cookie") || "";
  const match2 = cookie.match(/sb-[^-]+-auth-token=([^;]+)/);
  if (!match2) return null;
  try {
    const payload = JSON.parse(decodeURIComponent(match2[1]));
    const token = payload?.access_token;
    if (!token) return null;
    const supabase = getAdminClient(env2);
    const { data: { user }, error: error3 } = await supabase.auth.getUser(token);
    if (error3 || !user) return null;
    return { id: user.id };
  } catch {
    return null;
  }
...
async function requireAdmin(c) {
  ...
  const { data: profile3 } = await supabase.from("profiles").select("role").eq("id", auth.id).single();
...
function sortImages(rows) {
  ...
  return [...rows].sort((a, b) => a.is_primary === b.is_primary ? a.sort_order - b.sort_order : a.is_primary ? -1 : 1).map((r) => r.url);
}
__name(sortImages, "sortImages");
function mapProductImages(product) {
  const images = sortImages(product.product_images ?? null);
  const { product_images: _, ...rest } = product;
  return { ...rest, images: images.length ? images : product.images ?? [] };
}
__name(mapProductImages, "mapProductImages");
function mapProductImagesList(products) {
  return products.map((p) => mapProductImages(p));
}
__name(mapProductImagesList, "mapProductImagesList");

// src/routes/public.ts
var PUBLIC_KEYS = ["instagram_url", "whatsapp_number", "announcement_bar", "store_name", "store_email"];
var app = new Hono2();
app.get("/settings/public", async (c) => {
  const supabase = getAdminClient(c.env);
  const { data, error: error3 } = await supabase.from("settings").select("key, value").in("key", PUBLIC_KEYS);
...
app.get("/products", async (c) => {
  ...
  const supabase = getAdminClient(c.env);
  if (slug) {
    const { data: raw2, error: error4 } = await supabase.from("products").select("*, product_variants(*), product_images(url, sort_order, is_primary)").eq("slug", slug).eq("is_active", true).single();
    if (error4 || !raw2) return c.json({ error: "Not found" }, 404);
    return c.json(mapProductImages(raw2));
  }
  let query = supabase.from("products").select("*, product_variants(*), product_images(url, sort_order, is_primary)", { count: "exact" }).eq("is_active", true);
  if (category) {
  ...
  const { data, error: error3, count: count3 } = await query.order("sort_order");
  if (error3) return c.json({ error: error3.message }, 500);
  const list = mapProductImagesList(data ?? []);
  return c.json({ data: list, count: count3 ?? 0 });
});
app.get("/orders", async (c) => {
  ...
  const auth = await getAuthFromRequest(c.req.raw, c.env);
  const supabase = getAdminClient(c.env);
  const { data: orderByGuest } = await supabase.from("orders").select("id, order_number, status, total, tracking_number, tracking_url, courier_name, order_items(product_name, size_ml, quantity, total_price)").eq("order_number", orderNumber.trim()).eq("guest_email", email.trim()).single();
  if (orderByGuest) return c.json({ order: orderByGuest });
  if (auth?.id) {
    const { data: orderByUser } = await supabase.from("orders").select("id, order_number, status, total, tracking_number, tracking_url, courier_name, order_items(product_name, size_ml, quantity, total_price)").eq("order_number", orderNumber.trim()).eq("user_id", auth.id).single();
...
app.get("/auth/callback", async (c) => {
  ...
  if (code) {
    const supabase = getAdminClient(c.env);
    const { error: error3 } = await supabase.auth.exchangeCodeForSession(code);
    if (!error3) {
...
app.post("/coupons/validate", async (c) => {
  try {
    ...
    const supabase = getAdminClient(c.env);
    const { data: coupon, error: error3 } = await supabase.from("coupons").select("*").eq("code", String(code).toUpperCase().trim()).eq("is_active", true).single();
    if (error3 || !coupon) {
    ...
    const discount = coupon.type === "percentage" ? Math.min(subtotal * coupon.value / 100, coupon.max_discount ?? Infinity) : coupon.value;
    return c.json({
      valid: true,
      discount,
...
app.post("/newsletter", async (c) => {
  try {
    ...
    const supabase = getAdminClient(c.env);
    const { error: error3 } = await supabase.from("newsletter_subscribers").upsert(
      { email: trimmed, subscribed_at: (/* @__PURE__ */ new Date()).toISOString() },
      { onConflict: "email" }
    );
    if (error3) return c.json({ error: error3.message }, 500);
    return c.json({ success: true });
...
var public_default = app;

// src/lib/r2.ts
async function uploadToR2(env2, key, body, contentType) {
  const bucket = env2.R2_BUCKET;
  const baseUrl = (env2.NEXT_PUBLIC_R2_PUBLIC_URL || "").replace(/\/$/, "");
...
// src/routes/admin.ts
var slugify = /* @__PURE__ */ __name((s) => s.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, ""), "slugify");
var admin = new Hono2();
...
admin.get("/settings", async (c) => {
  const supabase = getAdminClient(c.env);
  const { data, error: error3 } = await supabase.from("settings").select("key, value");
  if (error3) return c.json({ error: error3.message }, 500);
  const obj = {};
  (data ?? []).forEach((r) => obj[r.key] = r.value ?? "");
  return c.json(obj);
...
admin.get("/products", async (c) => {
  ...
  const supabase = getAdminClient(c.env);
  let query = supabase.from("products").select("*, product_variants(*), product_images(url, sort_order, is_primary)", { count: "exact" });
  ...
  const { data: raw2, error: error3, count: count3 } = await query.order("sort_order").range(from, from + limit - 1);
  if (error3) return c.json({ error: error3.message }, 500);
  const data = mapProductImagesList(raw2 ?? []);
  return c.json({ data, count: count3 ?? 0, page, total_pages: count3 ? Math.ceil(count3 / limit) : 0 });
});
admin.post("/products", async (c) => {
  const body = await c.req.json();
  const { name, slug, tagline, description, short_description, inspired_by, category_type, fragrance_profile, top_notes, heart_notes, base_notes, rating, is_featured, is_active, meta_title, meta_description, sort_order, variants = [], category_ids = [], image_urls = [] } = body;
  ...
  for (const v of variants) {
    ...
    const sku = v.sku || `URS-${slugVal.replace(/-/g, "").slice(0, 8)}-${size_ml}`;
  ...
  for (let i = 0; i < image_urls.length; i++) {
    await supabase.from("product_images").insert({ product_id: productId, url: image_urls[i], sort_order: i, is_primary: i === 0 });
  }
  ...
  const mapped = full ? mapProductImagesList([full])[0] : null;
  return c.json(mapped ?? product, 201);
});
admin.get("/products/:id", async (c) => {
  const id = c.req.param("id");
  const supabase = getAdminClient(c.env);
  const { data, error: error3 } = await supabase.from("products").select("*, product_variants(*), product_images(url, sort_order, is_primary)").eq("id", id).single();
  if (error3 || !data) return c.json({ error: "Not found" }, 404);
  return c.json(mapProductImages(data));
});
admin.put("/products/:id", async (c) => {
  const id = c.req.param("id");
  const body = await c.req.json();
  const supabase = getAdminClient(c.env);
  const { name, slug, tagline, description, short_description, inspired_by, category_type, fragrance_profile, top_notes, heart_notes, base_notes, rating, is_featured, is_active, meta_title, meta_description, sort_order, variants = [], category_ids = [] } = body;
  const slugVal = slug != null ? String(slug).trim().toLowerCase().replace(/\s+/g, "-") : void 0;
  const { error: updateErr } = await supabase.from("products").update({
    ...name != null && { name: String(name) },
    ...slugVal != null && { slug: slugVal },
    ...tagline !== void 0 && { tagline: tagline ?? null },
    ...description !== void 0 && { description: description ?? null },
    ...short_description !== void 0 && { short_description: short_description ?? null },
    ...inspired_by !== void 0 && { inspired_by: inspired_by ?? null },
    ...category_type !== void 0 && { category_type: category_type ?? "unisex" },
    ...fragrance_profile !== void 0 && { fragrance_profile: fragrance_profile ?? null },
    ...
    ...is_active !== void 0 && { is_active: is_active !== false },
    ...meta_title !== void 0 && { meta_title: meta_title ?? null },
    ...meta_description !== void 0 && { meta_description: meta_description ?? null },
...
admin.delete("/products/:id", async (c) => {
  ...
  await supabase.from("product_categories").delete().eq("product_id", id);
  const { error: error3 } = await supabase.from("products").delete().eq("id", id);
  if (error3) return c.json({ error: error3.message }, 500);
...
admin.get("/products/:id/images", async (c) => {
  const id = c.req.param("id");
  const supabase = getAdminClient(c.env);
  const { data, error: error3 } = await supabase.from("product_images").select("*").eq("product_id", id).order("sort_order");
  if (error3) return c.json({ error: error3.message }, 500);
  const sorted = (data ?? []).sort((a, b) => a.is_primary === b.is_primary ? 0 : a.is_primary ? -1 : 1);
...
admin.post("/products/:id/images", async (c) => {
  ...
  const { data, error: error3 } = await supabase.from("product_images").insert({
    product_id: id,
    url: body.url,
...
admin.patch("/products/:id/images", async (c) => {
  ...
  if (body.is_primary !== void 0) updates.is_primary = body.is_primary;
  const { data, error: error3 } = await supabase.from("product_images").update(updates).eq("id", body.id).select().single();
  if (error3) return c.json({ error: error3.message }, 500);
  return c.json(data);
});
admin.delete("/products/:id/images", async (c) => {
  ...
  const supabase = getAdminClient(c.env);
  const { error: error3 } = await supabase.from("product_images").delete().eq("id", id);
  if (error3) return c.json({ error: error3.message }, 500);
  return c.body(null, 204);
});
...
admin.get("/categories", async (c) => {
  const supabase = getAdminClient(c.env);
  const { data: categories, error: error3 } = await supabase.from("categories").select("*").order("sort_order").order("name");
  if (error3) return c.json({ error: error3.message }, 500);
  const withCount = await Promise.all(
    (categories ?? []).map(async (cat) => {
      const { count: count3 } = await supabase.from("product_categories").select("*", { count: "exact", head: true }).eq("category_id", cat.id);
      return { ...cat, product_count: count3 ?? 0 };
    })
  );
...
admin.post("/categories", async (c) => {
  ...
  const supabase = getAdminClient(c.env);
  const { data, error: error3 } = await supabase.from("categories").insert({
    name: String(name).trim(),
    slug: slugVal || void 0,
    description: description ?? null,
...
admin.put("/categories/:id", async (c) => {
  ...
  const supabase = getAdminClient(c.env);
  const { name, slug, description, parent_id, sort_order, is_active, banner_image } = body ?? {};
  const slugVal = slug != null ? String(slug).toString().toLowerCase().trim().replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, "") : void 0;
  const { data, error: error3 } = await supabase.from("categories").update({
    ...name != null && { name: String(name).trim() },
    ...slugVal != null && { slug: slugVal },
    ...description !== void 0 && { description: description ?? null },
    ...parent_id !== void 0 && { parent_id: parent_id ?? null },
    ...typeof sort_order === "number" && { sort_order },
    ...is_active !== void 0 && { is_active: is_active !== false },
    ...banner_image !== void 0 && { banner_image: banner_image ?? null }
...
admin.delete("/categories/:id", async (c) => {
  ...
  const { error: error3 } = await supabase.from("categories").delete().eq("id", id);
  if (error3) return c.json({ error: error3.message }, 500);
  return c.body(null, 204);
});
admin.get("/orders", async (c) => {
  ...
  const { data, error: error3, count: count3 } = await query.order("created_at", { ascending: false }).range(from, from + limit - 1);
  if (error3) return c.json({ error: error3.message }, 500);
  return c.json({ data: data ?? [], count: count3 ?? 0, page, limit, total_pages: count3 ? Math.ceil(count3 / limit) : 0 });
});
admin.get("/orders/:id", async (c) => {
  const id = c.req.param("id");
  const supabase = getAdminClient(c.env);
  const { data, error: error3 } = await supabase.from("orders").select("*, order_items(*), shipping_address").eq("id", id).single();
  if (error3 || !data) return c.json({ error: "Not found" }, 404);
  return c.json(data);
});
admin.patch("/orders/:id", async (c) => {
  ...
  const { data, error: error3 } = await supabase.from("orders").update(updates).eq("id", id).select().single();
  if (error3) return c.json({ error: error3.message }, 500);
  return c.json(data);
});
admin.get("/inventory", async (c) => {
  ...
  const { data: variants, error: error3 } = await query.order("product_id").order("size_ml");
  if (error3) return c.json({ error: error3.message }, 500);
  let list = variants ?? [];
  if (low_stock_only) list = list.filter((v) => v.stock_quantity <= v.low_stock_threshold);
  return c.json(list);
});
admin.post("/inventory", async (c) => {
  const body = await c.req.json();
  const { variant_id, type, quantity_change, note } = body ?? {};
  if (!variant_id || !type || quantity_change === void 0) return c.json({ error: "variant_id, type (restock|adjustment), and quantity_change required" }, 400);
  ...
  const { error: updateErr } = await supabase.from("product_variants").update({ stock_quantity: quantity_after, updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", variant_id);
  if (updateErr) return c.json({ error: updateErr.message }, 500);
...
admin.get("/coupons", async (c) => {
  const supabase = getAdminClient(c.env);
  const { data, error: error3 } = await supabase.from("coupons").select("*").order("created_at", { ascending: false });
  if (error3) return c.json({ error: error3.message }, 500);
  return c.json(data ?? []);
});
admin.post("/coupons", async (c) => {
  ...
  const { data, error: error3 } = await supabase.from("coupons").insert({
    code: codeVal,
...
admin.get("/coupons/:id", async (c) => {
  const id = c.req.param("id");
  const supabase = getAdminClient(c.env);
  const { data, error: error3 } = await supabase.from("coupons").select("*").eq("id", id).single();
  if (error3 || !data) return c.json({ error: "Not found" }, 404);
  return c.json(data);
});
admin.put("/coupons/:id", async (c) => {
  ...
  const body = await c.req.json();
  const supabase = getAdminClient(c.env);
  const { data, error: error3 } = await supabase.from("coupons").update(body).eq("id", id).select().single();
  if (error3) return c.json({ error: error3.message }, 500);
  return c.json(data);
});
admin.delete("/coupons/:id", async (c) => {
  const id = c.req.param("id");
  const supabase = getAdminClient(c.env);
  const { error: error3 } = await supabase.from("coupons").delete().eq("id", id);
  if (error3) return c.json({ error: error3.message }, 500);
  return c.body(null, 204);
});
admin.get("/customers", async (c) => {
  ...
  const { data, error: error3, count: count3 } = await query.range(from, from + limit - 1);
  if (error3) return c.json({ error: error3.message }, 500);
  return c.json({ data: data ?? [], count: count3 ?? 0, page, total_pages: count3 ? Math.ceil(count3 / limit) : 0 });
});
admin.get("/contacts", async (c) => {
  const supabase = getAdminClient(c.env);
  const { data, error: error3 } = await supabase.from("contact_messages").select("*").order("created_at", { ascending: false });
  if (error3) return c.json({ error: error3.message }, 500);
  return c.json(data ?? []);
});
admin.patch("/contacts", async (c) => {
  ...
  if (!body?.id) return c.json({ error: "id required" }, 400);
  const supabase = getAdminClient(c.env);
  const { data, error: error3 } = await supabase.from("contact_messages").update({ read: body.read }).eq("id", body.id).select().single();
  if (error3) return c.json({ error: error3.message }, 500);
  return c.json(data);
});
admin.get("/reviews", async (c) => {
  ...
  const from = (page - 1) * limit;
  const supabase = getAdminClient(c.env);
  const { data, error: error3, count: count3 } = await supabase.from("reviews").select("*, products(name, slug)", { count: "exact" }).order("created_at", { ascending: false }).range(from, from + limit - 1);
  if (error3) return c.json({ error: error3.message }, 500);
  return c.json({ data: data ?? [], count: count3 ?? 0, page, total_pages: count3 ? Math.ceil(count3 / limit) : 0 });
});
admin.patch("/reviews/:id", async (c) => {
  ...
  const body = await c.req.json();
  const supabase = getAdminClient(c.env);
  const { data, error: error3 } = await supabase.from("reviews").update({ is_approved: body?.is_approved }).eq("id", id).select().single();
  if (error3) return c.json({ error: error3.message }, 500);
  return c.json(data);
});
admin.get("/newsletter", async (c) => {
  ...
  const from = (page - 1) * limit;
  const supabase = getAdminClient(c.env);
  const { data, error: error3, count: count3 } = await supabase.from("newsletter_subscribers").select("*", { count: "exact" }).order("subscribed_at", { ascending: false }).range(from, from + limit - 1);
  if (error3) return c.json({ error: error3.message }, 500);
  return c.json({ data: data ?? [], count: count3 ?? 0, page, total_pages: count3 ? Math.ceil(count3 / limit) : 0 });
});
...
app2.route("/api", public_default);
app2.route("/api/admin", admin_default);
app2.get("/api/health", (c) => c.json({ ok: true, service: "ursignature-api" }));
